...
/* Remove the current student that the Ta ta_tofree is serving and free its memory.
 */
void release_current_student(Ta *ta_tofree) {
	Student *student = ta_tofree->current_student;
	if (student != NULL) {
		free(student->name);
		free(student);
	}
...
char *print_currently_serving(Ta *ta_list) {
    ...
    Ta *curr = ta_list;
    if (ta_list == NULL) {
        count = asprintf(&str, "No TAs are currently working.\r\n");
        if (count == -1) {
            perror("asprintf in print_currently_serving for no Ta");
	    	exit(1);
        }
        return str;
    }
    while (ta_list != NULL) {
        ...
        if (ta_list->current_student != NULL) {
            num = asprintf(&str, "TA: %s is serving %s.\r\n", ta_list->name, ta_list->current_student->name);
        } else {
            num = asprintf(&str, "TA: %s has no student\r\n", ta_list->name);
        }
        if(num == -1) {
            perror("asprintf in print_currently_serving for Ta");
	    	exit(1);
        }
        count += num;
    ...
    while (curr != NULL) {
        ...
        if (curr->current_student != NULL) {
            strcat(str, " is serving ");
            strcat(str, curr->current_student->name);
            strcat(str, ".\r\n");
        } else {
            strcat(str, " has no student\r\n");
        }
        curr = curr->next;
    }
    return str;
...
/* Print a list of all the students in the queue.
 * This does not print students currently being served.
 */ 
char *print_full_queue(Student *stu_list) {
    ...
    // printf("Full Queue\n");
    while (stu_list != NULL) {
        int num = asprintf(&str, "Student %s:%s\r\n", stu_list->name, stu_list->course->code);
        if (num == -1) {
            perror("asprintf in print_full_queue for Student");
			exit(1);
        }
        count += num;
        stu_list = stu_list->next_overall;
    ...
    // we add a "\0" to the end.
    str = malloc(sizeof(char)*(count+1+strlen("Full Queue\r\n")));
    str[0] = '\0';
	strcat(str, "Full Queue\r\n");
    while (curr != NULL) {
        strcat(str, "Student ");
        strcat(str, curr->name);
        strcat(str, ":");
        strcat(str, curr->course->code);
        strcat(str, "\n");
